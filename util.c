...
/*
 * Modified by SevOne Inc. (www.sevone.com) on 20111117
 * - Changed int2str() to bgpint2str() to avoid name collision with similarly named mysql function in .so files.
 * - Fixed 1 byte buffer overrun in bgpint2str() by changing i-- to --i.
*/
...
int bgpint2str(uint32_t value, char* str)
{
    const int LEN = 11;
    char b[LEN];
    int i = LEN;
